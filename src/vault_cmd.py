...
def check_directory(dir, vault_path, config_path):
    """
...
def initialize(vault: Vault, erase=None, clipboard_TTL=None, auto_lock_TTL=None, hide_secret_TTL=None, rekey_vault=None, import_items=None, export=None, file_format='json'):
    # Some nice ascii art
    logo()

    # Set vault path at the global scope
    global_scope['db_file'] = vault.vault_path

    # Create the vault folder if it does not exists yet
    check_directory(dir_, vault.vault_path, vault.config_path)

    # Assess files integrity
    assess_integrity(vault.vault_path, vault.config_path)

    # Erase a vault if the user requests it
    if erase:
        erase_vault(vault.vault_path, vault.config_path)
        sys.exit()

    # Load config
    global_scope['conf'] = Config(vault.config_path)

    # Migration from Vault 1.x to Vault 2.x
    if global_scope['conf'].version.split('.')[0] == '1':
        migrate(vault_path=vault.vault_path.strip('.db'), config_path=vault.config_path)
        sys.exit()
...
def main():
    ...
    vault = Vault(config_path=args.config_location, vault_path=args.vault_location)
